 ...
 *
 * For no good reason, I'm also demonstrating a more obscure feature
 * of C++: if you put two string constants one after the other in a
 * source file with no intervening operator, the strings are
 * concatenated together to make one string constant.  It can be handy
 * if you have to spread a string constant across multiple lines,
 * e. g., if the string is very long (like a URL).
 *
